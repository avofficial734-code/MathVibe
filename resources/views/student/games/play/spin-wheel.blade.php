<x-app-layout>
            ...
            <div class="bg-gradient-to-br from-slate-800/40 to-slate-900/40 backdrop-blur-md rounded-3xl p-8 border border-white/10 shadow-2xl">
                <div class="text-center" id="gameContainer">
                    ...
                    <div id="startScreen" style="display: none;" class="py-20">
                        <div class="inline-block text-center max-w-md">
                            ...
                            <button onclick="spinWheelGame.startGame()" class="bg-gradient-to-r from-[#283B60] to-[#1A3263] hover:from-[#283B60] hover:to-white/10 text-white font-bold py-4 px-8 rounded-xl text-lg shadow-lg transition transform hover:scale-105 active:scale-95 touch-manipulation">
                                ▶️ Start Game
                            </button>
                        </div>
                    </div>
                    ...
                    <div id="playingState" style="display: none;" class="py-12">
                        ...
                        <div id="wheelSection" class="mb-8 flex flex-col items-center gap-4">
                            <p class="text-white/70 text-sm">Spin the wheel to choose a category</p>
                            
                            <!-- Wheel Container -->
                            <div class="flex justify-center items-center w-full">
                                <div class="relative max-w-[300px] w-full aspect-square">
                                    <!-- Pointer -->
                                    <div style="position: absolute; top: -5%; left: 50%; transform: translateX(-50%); z-index: 50;">
                                        <div style="width: 0; height: 0; border-left: 15px solid transparent; border-right: 15px solid transparent; border-top: 25px solid #f97316; margin: 0 auto;"></div>
                                    </div>
                                    
                                    <!-- Wheel -->
                                    <div id="spinWheel" style="width: 100%; height: 100%; position: relative; transform-origin: center center; margin: 0 auto;">
                                        <canvas id="wheelCanvas" width="300" height="300" style="display: block; width: 100%; height: 100%; object-fit: contain;"></canvas>
                                    </div>
                                </div>
                            </div>

                            <button id="spinBtn" onclick="spinWheelGame.spin()" class="bg-gradient-to-r from-yellow-400 to-orange-400 hover:from-yellow-500 hover:to-orange-500 text-slate-900 font-bold py-3 px-8 rounded-xl text-lg shadow-lg transition transform hover:scale-105 active:scale-95 touch-manipulation">
                                🎯 Spin
                            </button>
                            <div class="text-xs text-white/60 mt-1 hidden md:block">Press Space or Enter to spin</div>
                            <div class="text-xs text-white/60 mt-1 md:hidden">Tap button to spin</div>
                        ...
                        <div id="questionSection" style="display: none;">
                            ...
                            <div class="bg-gradient-to-br from-[#283B60]/20 to-blue-500/20 backdrop-blur-md rounded-2xl p-8 border border-[#283B60]/30 mb-8 max-w-2xl mx-auto">
                                ...
                                <div id="questionText" class="text-5xl md:text-6xl font-bold text-white mb-8"></div>
                    ...
                    <div id="gameOverScreen" style="display: none;" class="py-20">
                        <div class="inline-block text-center max-w-md">
                            ...
                            <div class="flex gap-4 justify-center flex-wrap">
                                <button onclick="spinWheelGame.initGame()" class="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-bold py-3 px-6 rounded-lg transition active:scale-95 touch-manipulation">
                                    🔄 Play Again
                                </button>
                ...
                <div id="hintModal" style="display: none;" class="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 px-4">
                    <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 max-w-md w-full border border-white/10">
                        ...
                        <button onclick="spinWheelGame.closeHint()" class="w-full bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-bold py-3 rounded-lg transition active:scale-95 touch-manipulation">
    ...
    <script>
        ...
        const spinWheelGame = {
            ...
            canvas: null,
            ctx: null,
            isDailyChallenge: @json($isDailyChallenge ?? false),
            ...
            spin() {
                ...
                // Calculate target angle to align segment center with top pointer
                // Pointer is at -90deg (Top)
                // Segment 0 center starts at -54deg (36deg clockwise from top)
                // To move center to top, we rotate -36deg (counter-clockwise) or +324deg (clockwise)
                // Formula: 360 - (selectedIndex * segmentAngle + segmentAngle/2)
                const targetAngle = selectedIndex * segmentAngle + segmentAngle / 2;
                const spinRotation = 360 * 5 + (360 - targetAngle);
                ...
                setTimeout(() => {
                    this.wheelRotation = spinRotation;
                    this.selectQuestion(selectedCat);
                    this.spinning = false;
                    document.getElementById('spinBtn').disabled = false;
                    wheel.style.transition = 'none';
                    // Normalize rotation to < 360 to prevent huge numbers
            ...
            selectQuestion(category) {
                // Update UI display
                document.getElementById('categoryDisplay').textContent = category.label;
                document.getElementById('categoryDisplay').style.color = category.color;
                ...
                if (categoryQuestions.length === 0) {
                    this.showError('No questions available for ' + category.label);
                    return;
                }
            ...
            showQuestion() {
                ...
                document.getElementById('categoryBadge').textContent = CATEGORIES[CATEGORIES.findIndex(c => 
                    c.topics.some(t => t === this.currentQuestion.topic))].label;
